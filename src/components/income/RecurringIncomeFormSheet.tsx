...
import type { RecurringIncomeTemplate, CreateRecurringIncomePayload } from '@/types';

export interface RecurringIncomeFormSheetRef {
  present: (template?: RecurringIncomeTemplate) => void;
}
...
export const RecurringIncomeFormSheet = forwardRef<RecurringIncomeFormSheetRef>((_props, ref) => {
  const sheetRef = useRef<BottomSheetModal>(null);
  const dispatch = useAppDispatch();
  const [createTpl, { isLoading: creating }] = useCreateRecurringIncomeMutation();
  const [updateTpl, { isLoading: updating }] = useUpdateRecurringIncomeMutation();
  const [deleteTpl, { isLoading: deleting }] = useDeleteRecurringIncomeMutation();
  ...
  const reset = useCallback((tpl?: RecurringIncomeTemplate) => {
    if (tpl) {
      setEditing(tpl);
      setTitle(tpl.title);
  ...
  useImperativeHandle(ref, () => ({
    present: (tpl?: RecurringIncomeTemplate) => {
      reset(tpl);
  ...
  const onSave = async () => {
    ...
    try {
      if (editing) {
        await updateTpl({ id: editing.id, body: { ...payload, active } }).unwrap();
  ...
  const onDelete = () => {
    ...
    Alert.alert('Şablonu sil', `"${editing.title}" silinsin mi?`, [
      { text: 'Vazgeç', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await deleteTpl(editing.id).unwrap();
  ...
  return (
    ...
    >
      <BottomSheetScrollView contentContainerClassName="gap-4 px-4 pb-10" keyboardShouldPersistTaps="handled">
        ...
        <Field label="Başlık">
          <Input value={title} onChangeText={setTitle} placeholder="Örn. Maaş" />
        </Field>
...
RecurringIncomeFormSheet.displayName = 'RecurringIncomeFormSheet';
