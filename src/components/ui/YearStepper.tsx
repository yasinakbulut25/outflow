...
export function YearStepper() {
  ...
  return (
    <View className="flex-row items-center gap-1 rounded-full border border-border bg-white p-1">
      <Pressable onPress={prevYear} hitSlop={6} accessibilityRole="button" accessibilityLabel="Önceki yıl" className="p-1 active:opacity-60">
        <Icon icon={ChevronLeft} size={18} color={colors.foreground} />
      </Pressable>
      <Text variant="h2" className="px-1 text-base">{year}</Text>
      <Pressable onPress={nextYear} hitSlop={6} accessibilityRole="button" accessibilityLabel="Sonraki yıl" className="p-1 active:opacity-60">
